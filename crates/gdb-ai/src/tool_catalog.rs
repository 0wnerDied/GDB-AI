...
const TOOLS: &[ToolProjection] = &[
    ToolProjection {
        ...
        description: "launch without session_id creates a session; keep result.session.session_id. Optional breakpoints install persistent software breakpoint locations before running; with stop omitted, launch runs to their first stop. With inspect it waits and returns those views at the resulting stop. Use create only for other pre-launch setup, or pass an existing session_id. list returns compact session summaries; status reads one session. Programs run unchanged with argument-only argv; patch their runtime first when needed. Relative paths use workspace roots; use first_instruction only for pre-run setup. Remote uses connect_remote with endpoint and optional executable.",

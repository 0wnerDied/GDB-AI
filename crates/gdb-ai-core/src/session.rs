        ...
        let mut worker = Self {
            capabilities: Arc::new(StdRwLock::new(SessionCapabilities {
                ...
                capabilities: BTreeMap::from([
                    ...
                    ("sandbox.mount_namespace".into(), capability(if backend.descriptor().sandboxed { CapabilityStatus::Supported } else { CapabilityStatus::Unsupported }, "session", vec![], "bubblewrap", 0)),
                    ("sandbox.network_namespace".into(), capability(if backend.descriptor().network_isolated { CapabilityStatus::Supported } else { CapabilityStatus::Unsupported }, "session", vec![], "bubblewrap", 0)),
                ...
                limitations: vec![
                    "PID namespace, cgroup, and seccomp isolation require a deployment supervisor".into(),
                ],
            })),
            backend,
            journal,
